...
wrk_set=39
hot_set="30 31 32 33 34 35 36 37 38"
...
for size in ${hot_set}; do
    ...
    for ((i = 0; i < ${iters}; ++i )); do 
        NVMSIZE=${nvmsize} DRAMSIZE=${dramsize} NVMOFFSET=0 DRAMOFFSET=0 \
            numactl -N0 -m0 -- ./gups-pebs ${thds} ${upds} ${wrk_set} ${obj_size} ${size} >> ${res}/${prefix}_single.txt;
        ...
        NVMSIZE=$((${nvmsize}/2)) DRAMSIZE=$((${dramsize}/2)) NVMOFFSET=0 DRAMOFFSET=0 \
            numactl -N0 -m0 -- ./gups-pebs ${thds2} ${upds} ${wrk_set2} ${obj_size} ${size2} >> ${res}/${prefix}_double1.txt &
        NVMSIZE=$((${nvmsize}/2)) DRAMSIZE=$((${dramsize}/2)) NVMOFFSET=$((${nvmsize}/2)) DRAMOFFSET=$((${dramsize}/2)) \
            numactl -N0 -m0 -- ./gups-pebs ${thds2} ${upds} ${wrk_set2} ${obj_size} ${size2} >> ${res}/${prefix}_double2.txt;
        wait;
    done
done
