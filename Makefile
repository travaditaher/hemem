...
run_eval_dynamic_hemem: all
	...
	${SETUP_CMD} \
	PREFIX=dynamic; \
	$(MAKE) run_flexkvs NVMSIZE=$${NVMSIZE1} DRAMSIZE=$${DRAMSIZE1} NVMOFFSET=$${NVMSIZE1} DRAMOFFSET=$${DRAMSIZE1} PRELOAD="${HEMEM_PRELOAD}" FLEXKV_HOT_FRAC=0.33 FLEXKV_SIZE=$${FLEXKV_SIZE} PREFIX=$${PREFIX}_HeMem & \
	FLEX_PID=$$!; \
