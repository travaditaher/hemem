...
run_bg_sw_tier: all
	...
	${RUN_PERF} \
	$(MAKE) run_flexkvs HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} NVMSIZE=$${NVMSIZE} DRAMSIZE=$${DRAMSIZE} PRELOAD="${HEMEM_PRELOAD}" \
		NVMOFFSET=0 DRAMOFFSET=0 FLEXKV_SIZE=$${FLEXKV_SIZE} PREFIX=$${PREFIX}_Isolated; \
	pkill flexkvs;\
	$(MAKE) run_flexkvs WAIT_BG=1 WAIT_SCRIPT="wait-gups.sh ${RES}/$${PREFIX}_gups_pebs.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} NVMSIZE=$${NVMSIZE} DRAMSIZE=$${DRAMSIZE} PRELOAD="${HEMEM_PRELOAD}" \
	...
	${KILL_PERF} \
	${RUN_PERF} \
	$(MAKE) run_flexkvs WAIT_BG=1 WAIT_SCRIPT="wait-gapbs.sh ${RES}/$${PREFIX}_gapbs.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} NVMSIZE=$${NVMSIZE} DRAMSIZE=$${DRAMSIZE} PRELOAD="${HEMEM_PRELOAD}" \
	...
	${KILL_PERF} \
	${RUN_PERF} \
	$(MAKE) run_flexkvs WAIT_BG=1 WAIT_SCRIPT="wait-bt.sh ${RES}/$${PREFIX}_bt.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} NVMSIZE=$${NVMSIZE} DRAMSIZE=$${DRAMSIZE} PRELOAD="${HEMEM_PRELOAD}" \
...
run_test_nodram_bg_sw_tier: all
	...
	pkill flexkvs;\
	$(MAKE) run_flexkvs WAIT_BG=1 WAIT_SCRIPT="wait-gups.sh ${RES}/$${PREFIX}_gups_pebs.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} DRAMSIZE=0 DRAMOFFSET=${DRAMSIZE} NVMSIZE=$${NVMSIZE1} NVMOFFSET=0 PRELOAD="${HEMEM_PRELOAD}" \
	...
	${KILL_PERF} \
	${RUN_PERF} \
	$(MAKE) run_flexkvs WAIT_BG=1 WAIT_SCRIPT="wait-gapbs.sh ${RES}/$${PREFIX}_gapbs.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} DRAMSIZE=0 DRAMOFFSET=${DRAMSIZE} NVMSIZE=$${NVMSIZE1} NVMOFFSET=0 PRELOAD="${HEMEM_PRELOAD}" \
	...
	${KILL_PERF} \
	${RUN_PERF} \
	$(MAKE) run_flexkvs  WAIT_BG=1 WAIT_SCRIPT="wait-bt.sh ${RES}/$${PREFIX}_bt.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} DRAMSIZE=0 DRAMOFFSET=${DRAMSIZE} NVMSIZE=$${NVMSIZE1} NVMOFFSET=0 PRELOAD="${HEMEM_PRELOAD}" \
...
run_test_bg_sw_tier: all
	...
	file=${RES}/$${PREFIX}_Isolated; \
	${RUN_PERF} \
	$(MAKE) run_flexkvs HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} DRAMSIZE=${DRAMSIZE} DRAMOFFSET=0 NVMSIZE=$${NVMSIZE} NVMOFFSET=0 PRELOAD="${HEMEM_PRELOAD}" \
		FLEXKV_SIZE=$${FLEXKV_SIZE} PREFIX=$${PREFIX}_Isolated; \
	pkill flexkvs;\
	$(MAKE) run_flexkvs WAIT_BG=1 WAIT_SCRIPT="wait-gups.sh ${RES}/$${PREFIX}_gups_pebs.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} DRAMSIZE=${DRAMSIZE} DRAMOFFSET=0 NVMSIZE=$${NVMSIZE} NVMOFFSET=0 PRELOAD="${HEMEM_PRELOAD}" \
		FLEXKV_SIZE=$${FLEXKV_SIZE} PREFIX=$${PREFIX}_gups & \
	FLEX_PID=$$!; \
	$(MAKE) run_gups_pebs HEMEM_MGR_START_CPU=2 NVMSIZE=$${NVMSIZE} DRAMSIZE=0 NVMOFFSET=$${NVMSIZE} DRAMOFFSET=${DRAMSIZE} \
		PRELOAD="${HEMEM_PRELOAD}" APP_SIZE=${GUPS_SIZE} PREFIX=$${PREFIX}& \
	wait $${FLEX_PID}; \
	pkill flexkvs;\
	pkill gups-pebs;\
	...
	${KILL_PERF} \
	${RUN_PERF} \
	$(MAKE) run_flexkvs WAIT_BG=1 WAIT_SCRIPT="wait-gapbs.sh ${RES}/$${PREFIX}_gapbs.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} DRAMSIZE=${DRAMSIZE} DRAMOFFSET=0 NVMSIZE=$${NVMSIZE} NVMOFFSET=0 PRELOAD="${HEMEM_PRELOAD}" \
		FLEXKV_SIZE=$${FLEXKV_SIZE} PREFIX=$${PREFIX}_gapbs & \
	FLEX_PID=$$!; \
	$(MAKE) run_gapbs HEMEM_MGR_START_CPU=2 NVMSIZE=$${NVMSIZE} DRAMSIZE=0 NVMOFFSET=$${NVMSIZE} DRAMOFFSET=${DRAMSIZE} \
		PRELOAD="${HEMEM_PRELOAD}" APP_SIZE=${GAPBS_SIZE} PREFIX=$${PREFIX} & \
	wait $${FLEX_PID}; \
	pkill flexkvs;\
	pkill bc;\
	...
	${KILL_PERF} \
	${RUN_PERF} \
	$(MAKE) run_flexkvs WAIT_BG=1 WAIT_SCRIPT="wait-bt.sh ${RES}/$${PREFIX}_bt.txt" HEMEM_MGR_START_CPU=0 HEMEM_START_CPU=${FLEXKV_CPUS_START} HEMEM_NUM_CORES=${FLEXKV_THDS} DRAMSIZE=${DRAMSIZE} DRAMOFFSET=0 NVMSIZE=$${NVMSIZE} NVMOFFSET=0 PRELOAD="${HEMEM_PRELOAD}" \
		FLEXKV_SIZE=$${FLEXKV_SIZE} PREFIX=$${PREFIX}_bt & \
	FLEX_PID=$$!; \
	$(MAKE) run_bt HEMEM_MGR_START_CPU=2 NVMSIZE=$${NVMSIZE} DRAMSIZE=0 NVMOFFSET=$${NVMSIZE} DRAMOFFSET=${DRAMSIZE} \
		PRELOAD="${HEMEM_PRELOAD}" BT_SIZE=${BT_SIZE} PREFIX=$${PREFIX} & \
	wait $${FLEX_PID}; \
	pkill flexkvs;\
	pkill bt.E;\
